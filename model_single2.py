...
class RCAN(nn.Module):
    def __init__(self, args):
        ...
        self.rgs2 = nn.Sequential(*[RG(32, num_rcab, reduction) for _ in range(5)])
        ...
        self.conv_basic = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.conv1 = nn.Conv2d(80, 64, kernel_size=1, padding=1)
    def forward(self, x):
        x = self.sf(x)
        residual = x     # 16
        residual = self.sub(residual)
        x = self.rgs1(x)
        x = self.sub(x)        # 第一次放大
        residual1=x    #16

        x1 = self.relu(x)
        x1=self.conv_basic16_64(x1)
        x1 = self.rgs2(x1)
        # x1 = self.conv1(x1)
        x1 =torch.cat((x1 ,residual ,residual1),1)
        x1 = self.relu(x1)
        x1 =self.conv_basic(x1)
